                                            ...
                                            <tbody>
                                                @forelse($categories as $category)
                                                    <tr>
                                                        ...
                                                        <td>{{ $category->name }}</td>
                                                        <td>
                                                            {{ $category->created_at ? $category->created_at->format('d/m/Y H:i') : 'Data não disponível' }}
                                                        ...
                                                        <td>
                                                            <div class="btn-group" role="group">
                                                                <button type="button" class="btn btn-sm btn-warning" 
                                                                        ...
                                                                        data-bs-target="#editEventTypeModal{{ $category->id }}">
                                                                    <i class="bi bi-pencil"></i>
                                                                </button>
                                                                <form action="{{ route('event.update', $category->id) }}" method="POST" class="d-inline">
                                                                    @csrf
                                                                    @method('DELETE')
                                                            ...
                                                            <!-- Modal de Edição -->
                                                            <div class="modal fade" id="editEventTypeModal{{ $category->id }}" tabindex="-1" aria-labelledby="editEventTypeModalLabel{{ $category->id }}" aria-hidden="true">
                                                                <div class="modal-dialog">
                                                                    <div class="modal-content">
                                                                        <form action="{{ route('event.update', $category->id) }}" method="POST">
                                                                            @csrf
                                                                            @method('PUT')
                                                                            <div class="modal-header">
                                                                                <h5 class="modal-title" id="editEventTypeModalLabel{{ $category->id }}">Editar Tipo de Evento</h5>
                                                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                                                                            </div>
                                                                            <div class="modal-body">
                                                                                <div class="mb-3">
                                                                                    <label for="edit_name{{ $category->id }}" class="form-label">Nome do Tipo de Evento</label>
                                                                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                                                                           id="edit_name{{ $category->id }}" name="name" 
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <form action="{{ route('category.add') }}" method="POST">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="eventTypeModalLabel">Cadastrar categoria</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                                </div>
                                <div class="modal-body">
                                    <div class="mb-3">
